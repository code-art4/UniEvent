...
const Event = ({ item, id }) => {
    ...
    >
        ...
        <h2 className='text-xl mb-2 md:text-left lg:text-center font-bold text-black translate-y-[-3rem] w-full text-center bg-black/20 left-0 py-2'>
                    {reduceCharacters(item?.title)}
                </h2>
        <div class="absolute top-[-1.3rem] left-[-5.3rem] w-0 h-0 
  border-l-[8rem] border-l-transparent
  border-b-[8rem] border-b-[green]/90
  border-r-[8rem] border-r-transparent" style={{
    transform: 'rotate(-45deg)'
  }}>
    <span className='absolute top-[3.2rem] left-[-2.5rem] text-center text-lg font-semibold'>{formatDate(item?.date)} </span>
</div>       

        <div className='w-full mt-[-2rem] pb-3'>
            <div className='md:w-[95%] w-full md:block flex flex-col items-center bg-white mx-auto rounded-lg px-3 py-2'>                
                ...
                <button className='mt-4 mx-auto w-max bg-[green]/90 text-white px-3 py-2 rounded flex cursor-pointer' onClick={() => Router.push(`/event/${id}`)}>View Event</button>
            </div>
        </div>
